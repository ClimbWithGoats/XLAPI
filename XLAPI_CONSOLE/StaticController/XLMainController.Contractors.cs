...
namespace XLAPI_CONSOLE.StaticController
{
    ...
    public static partial class XLMainController
    {
        ...
        {
            // Console.WriteLine($"Metoda {nameof(AddContractors)} działa na wątku o ID: {Environment.CurrentManagedThreadId}");
        ...
        {
            // Console.WriteLine($"Metoda {nameof(AddOrUpdateContractor)} działa na wątku o ID: {Environment.CurrentManagedThreadId}");
            ...
            if (IdResult == -1)
            {
                ...
                if (result != null && result.ResId == 0 && result.ResultObject != null)
                {
                    ...
                    XLZamkniecieKontrahentaInfo close = new XLZamkniecieKontrahentaInfo() { Tryb = 0 };
            ...
            {
                List<XLAtrKlasaNagInfoExt> contractorsAttributes = configuration.Attributes.Contractors;
        ...
        {
            // Console.WriteLine($"Metoda {nameof(AddContractorsSQL)} działa na wątku o ID: {Environment.CurrentManagedThreadId}");
            SetProccesing(guid, true);
        ...
        public static void AddOrUpdateContractorSQL(XLKontrahentSQLInfo crSQL)
        {
            ...
            // Console.WriteLine($"Metoda {nameof(AddOrUpdateContractorSQL)} działa na wątku o ID: {Environment.CurrentManagedThreadId}");
            //crSQL.GrpTyp = crSQL.GrpTyp==null ? crSQL.GrpTyp : 0; // grupy maja ujemne wartosci
            //crSQL.GrpFirma = crSQL.GrpFirma ?? 3;// crSQL.GIDFirma;
            //crSQL.GrpNumer =crSQL.GrpNumer?? 0;// crSQL.GIDFirma;
            ...
            else if (IdResult > 0)
            {
                ...
                if (DynamicResult != null)
                {
                    ...
                    {
                        XLModyfikacjaKntSQLInfo obj = new XLModyfikacjaKntSQLInfo();
                        ...
                        if (result != null && result.ResId == 0 && result.ResultObject != null)
                        {
                            ...
                            foreach (var property in filteredProperties)
                            {
                                ...
                                if (value != null && property.Name != null && !property.Name.Equals(nameof(obj.GIDTyp)) && !property.Name.Equals(nameof(obj.GIDFirma)) && !property.Name.Equals(nameof(obj.GIDNumer)) && !property.Name.Equals(nameof(obj.Wersja)))
                                {
                                    ...
                                    if (res != null && (!res.Equals(currentValue)))
                                    {
                                        ...
                                        var changedResult = PrepareObjectAndInvokeMethod<XLModyfikacjaKntSQLInfo>(obj, $"cdn_api.{nameof(XLModyfikacjaKntSQLInfo)}", nameof(Metody.XLZmienPoleKntSQL), ref newArgs);
                                    }
                                    args = newArgs;
                                }
                            }
                            ...
                            if (closeResult.ResId == 0)
                            {
                                resultMessage = "Edytowano kontrahenta" + crSQL.GIDNumer;
                            }
                        }
                    }
                }
